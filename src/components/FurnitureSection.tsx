...
export default function FurnitureSection({ onScrollToSection, onOpenConsultation }: FurnitureSectionProps) {
  const guarantees = [
    ...
    {
      icon: <Layers size={18} className="text-[#8B6F4E]" />,
      title: "Дизайнерский свет и текстильное оформление окон под ключ",
      desc: "Интеграция кураторского дизайнерского освещения, изготовление и навеска текстиля: шторы день-ночь, тюль, портьеры с эффектом Black Out (100% затемнение для спален). Также мы устанавливаем качественные карнизы строго нужных размеров под ваши помещения."
    },
    {
